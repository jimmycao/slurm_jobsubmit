...
#include "src/plugins/sched/dynalloc/msg.h"
...
static void *_msg_thread(void *no_data)
{
	...
	info("keeping IP and PORT in /tmp/ip.conf");
	/*keep IP and Port for ORTE/RAS*/
	write_local_ip_port(sched_port);
...
//		rc = allocate_node_rpc(request_node_num, node_range_list, &jobid,
